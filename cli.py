...
def _run(args: argparse.Namespace, core: Any) -> int:
    ...
    plan = core.collect_plan(overwrite_modified=overwrite)
    pruned = plan["pruned"]
    ...
    if not pruned and not modified:
        print("Nothing to do.")
        return 0
    ...
    if not args.yes and not _confirm(overwrite):
        print("Aborted.")
        return 1

    result = core.apply(overwrite_modified=overwrite)
    print("Done.")
...
def _confirm(overwrite: bool) -> bool:
    if overwrite:
        print(
